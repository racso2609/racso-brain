  ...
  return (
    <div className="relative w-full text-left">
      <button
        ...
        className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border border-border bg-card text-foreground hover:bg-accent/50 transition-colors text-sm font-medium shadow-sm min-w-0"
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <Building2 className="w-4 h-4 text-primary shrink-0" />
          <span className="truncate text-left block">{activeTenant?.name ?? "Seleccionar espacio"}</span>
        </div>
      ...
      {isOpen && (
        <div
          role="listbox"
          className="absolute left-0 mt-1.5 w-full min-w-[240px] max-w-xs rounded-lg border border-border bg-popover text-popover-foreground shadow-lg z-50 py-1.5 focus:outline-none"
        >
